...
export class FeeCalculatorService {
  ...
  async calculateHopFees(
    ...
    const isPostUpgrade = blockHeight >= this.upgradeBlock;
    ...
    const decimalsMap = await this.assetRegistry.getDecimalsBatch(Array.from(assetIdsSet));
    ...
    if (isPostUpgrade && routedTradeInputAssetIds.includes(this.H2O_ASSET_ID)) {
      ...
      if (h2oInput) {
        ...
        const normalizedH2OAmount = normalizeAmount(h2oInput.amount, h2oDecimals);
        if (this.H2O_OMNIPOOL_ATTRIBUTION_ENABLED) {
          if (feeAmountsRaw[this.H2O_ASSET_ID]) {
            feeAmountsRaw[this.H2O_ASSET_ID] = addNormalizedAmounts(
              feeAmountsRaw[this.H2O_ASSET_ID],
    ...
    for (const fee of swapFees) {
      ...
      let decimals = decimalsMap.get(assetId);
      if (decimals === undefined) {
        const fetchedDecimals = await this.assetRegistry.getDecimals(assetId);
        if (fetchedDecimals === null) {
      ...
      const normalizedAmount = normalizeAmount(rawAmount, decimals);
      if (feeAmountsRaw[assetId]) {
        feeAmountsRaw[assetId] = addNormalizedAmounts(feeAmountsRaw[assetId], normalizedAmount);
      } else {
        feeAmountsRaw[assetId] = normalizedAmount;
        feeAssetIds.push(assetId);
      }
