/**
 * Script to update liquidation continuous aggregates to include ASSET_RESERVE fees
 * Run with: node scripts/update-asset-reserve-aggregates.js
 *
 * This script:
 * 1. Drops existing liquidation_fees_* continuous aggregates
 * 2. Recreates them with ASSET_RESERVE in fees_by_type
...
async function updateLiquidationAggregates() {
  ...
  try {
    ...
    // Step 2: Recreate continuous aggregates with ASSET_RESERVE support
    console.log('Step 2: Creating updated continuous aggregates with ASSET_RESERVE support...');
    for (const config of liquidationIntervals) {
      console.log(`  Creating ${config.name}...`);

      // Create continuous aggregate with ASSET_RESERVE included
      await client.query(`
        ...
        SELECT
          ...
          -- Fees by type (now includes ASSET_RESERVE)
          jsonb_build_object(
            'LIQUIDATION_PENALTY',
            COALESCE(SUM(
              CASE
    ...
    console.log('✓ All aggregates recreated with ASSET_RESERVE support\n');
    ...
    console.log('\nAsset Reserve fees are now included in the fees_by_type breakdown.');
