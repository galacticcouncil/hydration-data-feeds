/**
 * Script to update liquidation continuous aggregates to include PEPL fees
 * Run with: node scripts/update-liquidation-aggregates.js
 *
 * This script:
 * 1. Drops existing liquidation_fees_* continuous aggregates
 * 2. Recreates them with PEPL_LIQUIDATION_PROFIT in fees_by_type
...
async function updateLiquidationAggregates() {
  ...
  try {
    ...
    // Step 2: Recreate continuous aggregates with PEPL support
    console.log('Step 2: Creating updated continuous aggregates with PEPL support...');
    for (const config of liquidationIntervals) {
      console.log(`  Creating ${config.name}...`);

      // Create continuous aggregate with PEPL_LIQUIDATION_PROFIT included
      await client.query(`
        ...
        SELECT
          ...
          -- Fees by type (now includes PEPL_LIQUIDATION_PROFIT)
          jsonb_build_object(
            'LIQUIDATION_PENALTY',
            COALESCE(SUM(
              CASE
    ...
    console.log('✓ All aggregates recreated with PEPL support\n');
    ...
    console.log('\nPEPL liquidation profits are now included in the fees_by_type breakdown.');
